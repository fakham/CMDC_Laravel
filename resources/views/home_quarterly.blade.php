...
<div class="container-fluid">
    ...
    <div class="card">
        <div class="header">
            <h2 class="title"><b>Quarterly</b></h2>
        </div>
        <div class="container">
            <div class="row">
                <div class="col-md-6">
        ...
        $.ajax({
               ...
               success:function(d){
                  ...
                  var types = [];
                    ...
                    var old = moment().startOf('day').subtract(4 ,'Q');
                    // console.log(old);
                    var n = moment(old).endOf('day').add(1 ,'Q');
                    for (var i = 0; i < 4; i++) {
                        old = moment().startOf('day').subtract(4 - i, 'Q');
                        // console.log(old);
                        n = moment(old).endOf('day').add(1 ,'Q');
                        nombres[i] = 0;
                        types[i] = old.format('MMMM') + ' ' + old.get('year'); //types[n] = i.format('MMMM') + ' ' + i.get('year');
               ...
               success:function(d){
                  ...
                  var old = moment().startOf('day').subtract(4 ,'Q');
                    ...
                    for (var i = 0; i < 4; i++) {
                        ...
                        n = moment(old).endOf('day').add(1 ,'Q');
                        nombres[i] = 0;
                        types[i] = old.format('MMMM') + ' ' + old.get('year');
        ...
        $.ajax({
               ...
               success:function(d){
                  ...
                  var types = [];
                    ...
                    var dateS = dateD === "" ? moment().add(-4, 'Q') : moment(dateD);
                    var dateE = dateF === "" ? moment() : moment(dateF);
                    var dateR = moment();
                    var n = 0;
                    for (var i = moment(dateS); i.isBefore(dateE); i.add(1, 'Q')) {
                        nombres.push(0);
                        for (var j = 0; j < d.jsonCharges.length; j++) {
                            dateR = moment(d.jsonCharges[j].date);
                            if (dateR.quarter() == i.quarter() && dateR.isSame(i, 'year')) {
                                nombres[n] += parseFloat(d.jsonCharges[j].prix);
                            }
                        }

                        types[n] = i.format('MMMM') + ' ' + i.get('year');
                        n++;
                    }
        ...
        $.ajax({
            ...
            success:function(d){
                ...
                for (var i = moment(dateS); i.isBefore(dateE); i.add(1, 'Q')) {
                    nombres.push(0);
                    for (var j = 0; j < d.jsonRecettes.length; j++) {
                        dateR = moment(d.jsonRcettes[j].date);
                        if (dateR.quarter() == i.quarter() && dateR.isSame(i, 'year')) {
